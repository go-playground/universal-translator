package xog

import (
	"math"

	"github.com/go-playground/universal-translator/resources/locales"
)

type xog struct {
	locale  string
	plurals []locales.PluralRule
}

// New returns a new instance of translator for the 'xog' locale
func New() locales.Translator {
	return &xog{
		locale:  "xog",
		plurals: []locales.PluralRule{2, 6},
	}
}

// Locale returns the current translators string locale
func (t *xog) Locale() string {
	return t.locale
}

// Plurals returns the list of plurals associated with 'xog'
func (t *xog) Plurals() []locales.PluralRule {
	return t.plurals
}

// cardinalPluralRule returns the PluralRule given 'num' and digits/precision of 'v' for 'xog'
func (t *xog) cardinalPluralRule(num float64, v uint64) locales.PluralRule {

	n := math.Abs(num)

	if n == 1 {
		return locales.PluralRuleOne
	}

	return locales.PluralRuleOther
}
